...
export class Doctor {
    public run(workspace: string = process.cwd()): DoctorCheck[] {
        ...
        for (const [name, command] of [['Node.js', 'node'], ['Git', 'git'], ['npm', process.platform === 'win32' ? 'npm.cmd' : 'npm'], ['Docker', 'docker']] as const) {
            const result = spawn.sync(command, ['--version'], { encoding: 'utf8', shell: false, windowsHide: true }); checks.push({ name, status: result.status === 0 ? 'pass' : name === 'Docker' ? 'warn' : 'fail', detail: result.status === 0 ? String(result.stdout || result.stderr).trim() : 'Not available' });
        }
